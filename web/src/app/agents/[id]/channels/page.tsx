...
// ImOwnerClaimSection manages the fail-closed IM owner-identity claim flow:
// list each connected IM channel, show its claimed ownerImIds, and offer
// Claim (mint code) / Rebind (void + new code) / Unbind (remove one ID).
// See docs/superpowers/specs/2026-06-20-im-channel-admin-gate.md §6.
function ImOwnerClaimSection({
  agentId,
  channels,
}: {
  agentId: string;
  ...
  const t = useT();
  const [ownerImIds, setOwnerImIds] = useState<Record<string, string[]>>({});
  ...
  const refresh = useCallback(() => {
    ...
    getAgentConfig(agentId)
      .then((cfg) => setOwnerImIds(cfg?.ownerImIds || {}))
      .catch(() => {})
