...
// user. Credentials live at ~/.lununda/users/{userID}/credentials.json and
...
func legacyDeriveKey() []byte {
	...
	hash := sha256.Sum256([]byte("lununda:" + hostname + ":" + home))
	return hash[:]
}
...
func deriveKeyForUser(userID, passphrase string) []byte {
	...
	var seed string
	if passphrase != "" {
		seed = "lununda:user:" + userID + ":pp:" + passphrase
	} else {
		hostname, _ := os.Hostname()
		home, _ := os.UserHomeDir()
		seed = "lununda:user:" + userID + ":host:" + hostname + ":" + home
	}
