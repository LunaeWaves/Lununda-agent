...
// sandbox backend. Read at process start from LUNUNDA_* environment
// variables — there is no config file. Everything user-facing
// (providers, channels, agents) lives in the database.
//
// Set these as `LUNUNDA_<UPPER_SNAKE_CASE>` (or the explicit name in
...
type EnvGateway struct {
	Port int    // LUNUNDA_PORT       — default 18953
	Bind string // LUNUNDA_BIND       — "loopback" (default) or "all"
}

type EnvStorage struct {
	Type        string // LUNUNDA_STORAGE_TYPE  — "sqlite" (default) or "postgres"
	DSN         string // LUNUNDA_STORAGE_DSN   — empty = sqlite at $LUNUNDA_HOME/lununda.db
	AutoMigrate bool   // LUNUNDA_STORAGE_AUTO_MIGRATE — default true
}

type EnvSandbox struct {
	Enabled         bool   // LUNUNDA_SANDBOX_ENABLED
	Backend         string // LUNUNDA_SANDBOX_BACKEND  — "docker", "e2b", or "boxlite"
	Image           string // LUNUNDA_SANDBOX_IMAGE
	E2BKey          string // E2B_API_KEY
	BoxliteURL      string // LUNUNDA_SANDBOX_BOXLITE_URL — full base URL e.g. https://api.boxlite.ai/v1
	BoxliteClientID string // LUNUNDA_SANDBOX_BOXLITE_CLIENT_ID — default "default"
	BoxliteKey      string // BOXLITE_API_KEY — apikey sent as Authorization: Bearer
	BoxlitePrefix   string // LUNUNDA_SANDBOX_BOXLITE_PREFIX — workspace prefix, default "default"
}

type EnvLog struct {
	Level string // LUNUNDA_LOG_LEVEL — "debug" / "info" / "warn" / "error"
}

// LoadEnv reads the bootstrap configuration from LUNUNDA_* environment
...
func LoadEnv() *EnvConfig {
	...
	if v := os.Getenv("LUNUNDA_PORT"); v != "" {
		if p, err := strconv.Atoi(v); err == nil {
			cfg.Gateway.Port = p
		}
	}
	if v := os.Getenv("LUNUNDA_BIND"); v != "" {
		cfg.Gateway.Bind = v
	}

	if v := os.Getenv("LUNUNDA_STORAGE_TYPE"); v != "" {
		cfg.Storage.Type = v
	}
	if v := os.Getenv("LUNUNDA_STORAGE_DSN"); v != "" {
		cfg.Storage.DSN = v
	}
	if v := os.Getenv("LUNUNDA_STORAGE_AUTO_MIGRATE"); v != "" {
		cfg.Storage.AutoMigrate = v == "true" || v == "1"
	}

	if v := os.Getenv("LUNUNDA_SANDBOX_ENABLED"); v != "" {
		cfg.Sandbox.Enabled = v == "true" || v == "1"
	}
	if v := os.Getenv("LUNUNDA_SANDBOX_BACKEND"); v != "" {
	...
	if v := os.Getenv("LUNUNDA_SANDBOX_IMAGE"); v != "" {
		cfg.Sandbox.Image = v
	}
	if v := os.Getenv("E2B_API_KEY"); v != "" {
		cfg.Sandbox.E2BKey = v
	}
	if v := os.Getenv("LUNUNDA_SANDBOX_BOXLITE_URL"); v != "" {
		cfg.Sandbox.BoxliteURL = v
	}
	if v := os.Getenv("LUNUNDA_SANDBOX_BOXLITE_CLIENT_ID"); v != "" {
		cfg.Sandbox.BoxliteClientID = v
	}
	if v := os.Getenv("BOXLITE_API_KEY"); v != "" {
		cfg.Sandbox.BoxliteKey = v
	}
	if v := os.Getenv("LUNUNDA_SANDBOX_BOXLITE_PREFIX"); v != "" {
		cfg.Sandbox.BoxlitePrefix = v
	}

	if v := os.Getenv("LUNUNDA_LOG_LEVEL"); v != "" {
		cfg.Log.Level = v
	}
	return cfg
}

// applyObjectStoreEnv reads LUNUNDA_OBJECT_STORE_* env vars into cfg.
func applyObjectStoreEnv(cfg *Config) {
	read := func(key string) string { return os.Getenv("LUNUNDA_OBJECT_STORE_" + key) }
	if v := read("TYPE"); v != "" {
...
func ScrubBootSecrets() {
	keys := []string{
		"LUNUNDA_STORAGE_DSN",
		"LUNUNDA_OBJECT_STORE_TYPE",
		"LUNUNDA_OBJECT_STORE_LOCAL_ROOT",
		"LUNUNDA_OBJECT_STORE_REGION",
		"LUNUNDA_OBJECT_STORE_BUCKET",
		"LUNUNDA_OBJECT_STORE_PREFIX",
		"LUNUNDA_OBJECT_STORE_ACCESSKEY",
		"LUNUNDA_OBJECT_STORE_SECRETKEY",
		"LUNUNDA_OBJECT_STORE_ACCOUNTID",
		"LUNUNDA_OBJECT_STORE_ENDPOINT",
		"LUNUNDA_OBJECT_STORE_USESSL",
		"LUNUNDA_OBJECT_STORE_ALIYUN_INTERNAL",
		"BOXLITE_API_KEY",
		"E2B_API_KEY",
	}
	for _, k := range keys {
...
// by gateway boot to layer LUNUNDA_OBJECT_STORE_* on top of the DB-
