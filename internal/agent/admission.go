...
// owner-private: only the owner is admitted. `/claim` bypasses admission
// so the owner can bind their IM identity (via /claim <code>) before
// being recognized as owner.
//
...
// isOpenCommand reports whether text is the /claim slash command form —
// the only command allowed before the owner is recognized.
