...
type Registry struct {
	...
	// skills/ subdir (~/.lununda/users/<uid>/). A write to relative
...
// the OS reaps zombies when the Lununda Agent process exits anyway.
...
//
// Installs that explicitly opt in with LUNUNDA_ALLOW_HOST_EXEC=1
// additionally get a `host_exec` escape hatch so the agent can help
// with operator-environment tasks (lununda upgrade, ~/Downloads
