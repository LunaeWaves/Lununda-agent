...
func TestIsSensitiveEnvKey(t *testing.T) {
	cases := []struct {
		...
		{"LUNUNDA_STORAGE_DSN", true},
		{"LUNUNDA_OBJECT_STORE_ACCESSKEY", true},
		{"LUNUNDA_OBJECT_STORE_SECRETKEY", true},
		{"LUNUNDA_OBJECT_STORE_BUCKET", true},
		{"LUNUNDA_OBJECT_STORE_ALIYUN_INTERNAL", true},
		{"LUNUNDA_SANDBOX_BOXLITE_URL", true},
		...
		{"LUNUNDA_HOME", false},
		{"LUNUNDA_LOG_LEVEL", false},
		{"LUNUNDA_DEPLOY", false},
		{"LUNUNDA_ALLOW_HOST_EXEC", false},
	}
	for _, c := range cases {
...
func TestScrubSensitiveEnv(t *testing.T) {
	in := []string{
		"PATH=/usr/bin",
		"HOME=/home/x",
		"LUNUNDA_STORAGE_DSN=postgres://user:pw@host/db",
		"LUNUNDA_OBJECT_STORE_ACCESSKEY=AKIAEXAMPLE",
		"LUNUNDA_OBJECT_STORE_SECRETKEY=secret",
		"ANTHROPIC_API_KEY=sk-ant-x",
		"AWS_ACCESS_KEY_ID=AKIA",
		"LUNUNDA_HOME=/var/lib/lununda",
		"LANG=en_US.UTF-8",
...
func TestBuildSubprocessEnvOverridesSkillKeys(t *testing.T) {
	...
	t.Setenv("LUNUNDA_STORAGE_DSN", "must-be-stripped")
	...
	for _, kv := range out {
		...
		if len(kv) >= len("LUNUNDA_STORAGE_DSN=") && kv[:len("LUNUNDA_STORAGE_DSN=")] == "LUNUNDA_STORAGE_DSN=" {
			sawDSN = true
		}
	}
	if !sawFalKey {
	...
	if sawDSN {
		t.Errorf("LUNUNDA_STORAGE_DSN must be scrubbed; leaked in %v", out)
	}
}
