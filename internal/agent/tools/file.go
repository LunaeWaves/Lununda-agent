...
// admin-managed global skills directory (~/.lununda/skills/). Reads are
...
// agent on the host.
const globalSkillsDirSuffix = "/.lununda/skills"

// errGlobalSkillsDirWrite is returned when write_file targets
// ~/.lununda/skills/ from inside an agent chat. The message tells the model
// exactly how to recover.
var errGlobalSkillsDirWrite = fmt.Errorf("access denied: ~/.lununda/skills/ is the admin-managed global skills directory. To create a new skill, load the \"skill-creator\" skill and follow its workflow (it scaffolds into this agent's private skills dir). To install an existing one, use the install_skill tool")
...
// when it falls under one of the Lununda Agent-managed roots
// (~/.lununda/...) — those are runtime internals and should keep
...
func hostHomePath(path string) (string, bool) {
	...
	if path == "~" || strings.HasPrefix(path, "~/") {
		// Sandbox-only / Lununda Agent-internal subtrees: skip host expansion
		...
		//   ~/.lununda/... — runtime internals (db, workspaces, …)
		//   ~/.agents/...   — sandbox bind-mount target for npx skills.
		//                     Host has these at <LUNUNDA_HOME>/users/<uid>/skills/,
		...
		if strings.HasPrefix(path, "~/.lununda") || strings.HasPrefix(path, "~/.agents") {
			return "", false
		}
	...
	if strings.HasPrefix(path, "/Users/") || strings.HasPrefix(path, "/home/") {
		// Refuse Lununda Agent-internal subpaths even when the chatter
...
// admin-managed ~/.lununda/skills/ directory. Works across user home
...
func makeReadFile(r *Registry) ToolFunc {
	return func(ctx context.Context, rawArgs json.RawMessage) (string, error) {
		...
		if r.systemFileStore != nil && r.agentID != "" && basenameIsSystemFile(args.Path) {
			...
			// metadata dir (e.g. ~/.lununda/agents/<id>/agent) which
...
func registerSandboxedFile(r *Registry, ex sandbox.Executor) {
	r.Register("read_file", "Read the contents of a file", map[string]interface{}{
		...
		// /data/.lununda/workspaces/<id>/IDENTITY.md still hits the DB.
		if r.systemFileStore != nil && r.agentID != "" && basenameIsSystemFile(args.Path) {
			name := filepath.Base(filepath.Clean(args.Path))
			if data, err := r.readSystemFileForUser(ctx, r.systemFileUserID(name), name); err == nil {
				return string(data), nil
