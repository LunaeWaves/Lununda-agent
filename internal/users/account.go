...
// exist purely to give external end-users a stable lununda user_id so
...
// always resolves to the same lununda user_id, so retries are safe.
//
...
// EnsureAppUser returns the lununda user representing (apikeyID, externalID),
...
func (a *Accounts) EnsureAppUser(ctx context.Context, apikeyID, externalID, displayName string) (*Account, error) {
	...
	rec := &store.UserRecord{
		ID:           id,
		Username:     "ext:" + syn,
		Email:        syn + "@external.lununda.local",
		PasswordHash: "",
