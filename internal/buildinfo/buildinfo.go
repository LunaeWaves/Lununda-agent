...
// (rather than cmd/lununda/main) so internal/* code — notably the agent
...
// Version is the Lununda Agent release tag (e.g. "v0.4.2") set by the
...
// IsHostedDeploy reports whether this lununda process is running in a
// hosted/multi-tenant deployment (cloud) versus a self-hosted single-
// operator install. Driven by the LUNUNDA_DEPLOY env var:
//
//	LUNUNDA_DEPLOY=hosted        → IsHostedDeploy() == true
//	LUNUNDA_DEPLOY=self-hosted   → false
...
// the most common case (developer running lununda on their own
...
// osDeployVar reads LUNUNDA_DEPLOY and normalizes it. Lowercased so
// casing variations don't silently bypass the hosted flag.
func osDeployVar() string {
	return strings.ToLower(strings.TrimSpace(os.Getenv("LUNUNDA_DEPLOY")))
}
...
// operator has explicitly opted in via LUNUNDA_ALLOW_HOST_EXEC=1
...
// flow, `lununda upgrade`, `~/Downloads` triage) sets the env var
...
func IsHostExecAllowed() bool {
	...
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LUNUNDA_ALLOW_HOST_EXEC")))
