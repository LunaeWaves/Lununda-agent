...
Bridge OpenClaw TypeScript plugins to Lununda Agent's JSON-RPC protocol.

## How it works

```
Lununda Agent Gateway ←→ JSON-RPC (stdin/stdout) ←→ openclaw-proxy ←→ OpenClaw Plugin (JS/TS)
```

The proxy loads an OpenClaw plugin, captures its `register()` calls (tools, channels, etc.), and exposes them via Lununda Agent's JSON-RPC protocol.

## Supported capabilities

| OpenClaw API | Lununda Agent RPC | Status |
|---|---|---|
...
Lununda Agent will start the proxy as a subprocess, which loads the OpenClaw plugin and bridges all registered tools/channels.
