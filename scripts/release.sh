...
echo "⚡ Building Lununda Agent ${VERSION}"
echo ""
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # Asset name is intentionally version-less (e.g. lununda_linux_amd64.tar.gz)
    # so the in-binary `lununda upgrade` command (cmd_version.go) can find
...
echo ""
echo "Or use gh CLI:"
echo "  gh release create ${VERSION} ${DIST_DIR}/*.tar.gz ${DIST_DIR}/*.zip ${DIST_DIR}/checksums.txt --title \"Lununda Agent ${VERSION}\" --generate-notes"
