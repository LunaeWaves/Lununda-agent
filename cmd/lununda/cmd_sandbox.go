...
func sandboxCreateCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&image, "image", "ghcr.io/lunaewaves/lununda-sandbox:latest", "Docker image to use")
	return cmd
}
...
func sandboxListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			listCmd := exec.Command("docker", "ps", "--filter", "label=lununda=sandbox", "--format",
