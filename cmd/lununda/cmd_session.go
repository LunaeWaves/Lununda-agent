...
from the LOCAL lununda.db — "online" sessions need their DB pulled
down first (rsync ~/.lununda/lununda.db from the host).

Examples:
  lununda session export http://localhost:18953/agents/agt_xxx/chat/s-yyy/
  lununda session export https://app.lununda.ai/agents/agt_xxx/chat/s-yyy -o /tmp/run.json`,
	...
	cmd.Flags().StringVarP(&outputPath, "output", "o", "",
		"output JSON path (default ~/.lununda/logs/<sessionKey>.json)")
	cmd.Flags().StringVar(&dbPath, "db", "",
		"sqlite DB path (default ~/.lununda/lununda.db)")
	return cmd
}
...
// dbPath defaults to ~/.lununda/lununda.db with the same pragma
...
func openStoreAt(dbPath string) (*store.DBStore, func(), error) {
	if dbPath == "" {
		...
		dbPath = filepath.Join(home, ".lununda", "lununda.db")
	}
...
// ~/.lununda/logs/<sessionKey>.json. Per-session filename so a
...
func resolveOutputPath(explicit, sessionKey string) (string, error) {
	...
	return filepath.Join(home, ".lununda", "logs", sessionKey+".json"), nil
}
